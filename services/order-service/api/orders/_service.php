...
{
    $stmt = $pdo->prepare('SELECT ID, IDKHACHHANG, NGAYTHANHTOAN, GHICHU, TREMOVE, PAYMENT_METHOD, PAYMENT_STATUS FROM hoadonthanhtoan WHERE ID = :order_id AND IDKHACHHANG = :user_id LIMIT 1');
    $stmt->execute([
        'order_id' => $orderId,
        'user_id' => $userId,
    ]);
...
{
    $stmt = $pdo->prepare('SELECT ID, IDKHACHHANG, NGAYTHANHTOAN, GHICHU, TREMOVE, PAYMENT_METHOD, PAYMENT_STATUS FROM hoadonthanhtoan WHERE ID = :order_id LIMIT 1');
